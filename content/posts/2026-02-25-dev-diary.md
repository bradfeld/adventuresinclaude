...
Over in [Freshell](https://github.com/bradfeld/freshell) - a local-first file server I'm building - a security fix landed for the `/local-file` endpoint. The `allowedFilePaths` sandbox wasn't being enforced, which meant the endpoint could serve any file on the machine. The kind of bug where the security model exists in the config but wasn't actually being checked in the code path.
