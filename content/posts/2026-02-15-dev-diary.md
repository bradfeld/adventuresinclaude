...
Getting to 1.0.0 meant a chain of cleanup tickets. I used the new `/start chain` command - which I'd literally just built the day before as PLA-526 - to process ten CEOS tickets autonomously. Update the GitHub issue templates with all 17 skills (they still listed five). Add data format specs for the eight newer data types. Extract the Skill Structure Contract from CLAUDE.md into publishable docs. Delete four stale feature branches. Fix the skill-authoring guide that still called three shipped skills "future."
...
`git tag v1.0.0 && git push upstream v1.0.0 --tags`. Then a proper GitHub release with a changelog. Seventeen skills, public repo, MIT license.
